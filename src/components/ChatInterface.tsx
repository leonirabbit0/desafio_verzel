...
export default function ChatInterface() {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      ...
      if (!currentSessionId) {
        currentSessionId = `visitor_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      try {
        const response = await fetch(
          `http://127.0.0.1:8000/get_messages?session_id=${currentSessionId}`
        );
        ...
        if (data.messages && data.messages.length > 0) {
          ...
          setMessages([
            { role: "assistant", content: "Olá! Sou o Roberto, da Verzel. Como posso te ajudar hoje?" }
          ]);
        }
      } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/input_message?message_received=${encodeURIComponent(
          input
  ...
  if (loading) {
    return (
      <div style={styles.loading}>
        <Loader2 style={{ width: '40px', height: '40px', color: '#2563eb' }} className="animate-spin" />
        <p style={{ marginTop: '16px', color: '#6b7280', fontWeight: '500' }}>Carregando conversa...</p>
      </div>
    );
  }
